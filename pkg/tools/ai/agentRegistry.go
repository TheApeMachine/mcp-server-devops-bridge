...
func (mb *MessageBus) Subscribe(agentID, topic string) error {
	...
	// Check if agent already subscribed to avoid duplicates
	for _, subscriber := range mb.subscribers[topic] {
		if subscriber == agentID {
			// Agent already subscribed, no need to add again
			return nil
		}
	}
...
func (mb *MessageBus) Publish(msg Message) error {
	mb.mutex.Lock() // Using a write lock since we might modify the channels map
	channel, exists := mb.channels[msg.Topic]

	// Create the topic if it doesn't exist
	if !exists {
		channel = make(chan Message, 100) // Buffered channel
		mb.channels[msg.Topic] = channel
		mb.subscribers[msg.Topic] = []string{} // Initialize with empty subscribers
	}
	mb.mutex.Unlock()
